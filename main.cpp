...
enum class WeaponType { LASER, BULLET, HORIZONTAL_LASER, COUNT };
...
public:
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt)
	{
		...
		baseDamage = dmg;
		type = wt;
		if (wt == WeaponType::HORIZONTAL_LASER) {
			lifeTime = 0.5f;
		}
	}
	bool Update(float dt) {
		transform.position = Vector2Add(transform.position, Vector2Scale(physics.velocity, dt));
		if (type == WeaponType::HORIZONTAL_LASER) {
			lifeTime -= dt;
	...
	void Draw() const {
		...
		else if (type == WeaponType::HORIZONTAL_LASER) {
			static constexpr float LASER_WIDTH = 60.f;
			static constexpr float LASER_HEIGHT = 8.f;
			Rectangle lr = { transform.position.x - LASER_WIDTH * 0.5f, transform.position.y - LASER_HEIGHT * 0.5f, LASER_WIDTH, LASER_HEIGHT };
	...
	float GetRadius() const {
		...
		if (type == WeaponType::HORIZONTAL_LASER) return 8.f;
		return 2.f;
	}
	...
	float speed)
{
	...
	else if (wt == WeaponType::HORIZONTAL_LASER) {
		Vector2 hvel{ 0, -speed * 0.4f };
		return Projectile(pos, hvel, 40, wt);
	}
	else {
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
...

			{
				...
				const int outline = 3;
				int centerY = Renderer::Instance().Height() / 2;

				if (player->IsAlive() && player->GetHP() > 0) {
					DrawText(TextFormat("HP: %d", player->GetHP()), 20 + outline, centerY - fontSize - 10 + outline, fontSize, WHITE);
					DrawText(TextFormat("HP: %d", player->GetHP()), 20, centerY - fontSize - 10, fontSize, GREEN);

					DrawText(TextFormat("Destroyed: %d", destroyedEnemies), 20 + outline, centerY + 10 + outline, fontSize, WHITE);
					DrawText(TextFormat("Destroyed: %d", destroyedEnemies), 20, centerY + 10, fontSize, RED);

					const int weaponFontSize = 36;
					const int weaponY = centerY + fontSize + 40;
					Color selectedColor = YELLOW;
					Color normalColor = GRAY;
					const char* weaponNames[] = { "Laser", "Bullets", "Horizontal" };
					for (int i = 0; i < 3; ++i) {
						int y = weaponY + i * (weaponFontSize + 8);
						Color color = (currentWeapon == static_cast<WeaponType>(i)) ? selectedColor : normalColor;
						DrawText(weaponNames[i], 20 + outline, y + outline, weaponFontSize, WHITE);
						DrawText(weaponNames[i], 20, y, weaponFontSize, color);
					}
					...
					const char* lostText = "LOST";
					int lostFontSize = 120;
					int textWidth = MeasureText(lostText, lostFontSize);
					int textX = Renderer::Instance().Width() / 2 - textWidth / 2;
					int textY = Renderer::Instance().Height() / 2 - lostFontSize / 2;

					if (lostTimer < 5.0f) {
						if (static_cast<int>(lostTimer * 2) % 2 == 0) {
							DrawText(lostText, textX, textY, lostFontSize, RED);
						}
					} else {
						DrawText(lostText, textX, textY, lostFontSize, RED);
					}
			...
			if (destroyedEnemies == 10) {
				...
				if (!changed) {
					player->ChangeTexture("spaceship2.png");
					changed = true;
				}
			}
				...
				static bool changed = false;
				if (changed) {
					player->ChangeTexture("spaceship1.png");
					changed = false;
				}
			}
		}
...
private:
	...
	static constexpr size_t MAX_AST = 400;
